//! Cost enforcement guardrails for LLM usage.
//!
//! Tracks daily spending and hourly action rates, enforcing configurable
//! limits to prevent runaway agents from burning through API credits.
//!
...
impl CostGuard {
    ...
    pub async fn check_allowed(&self) -> Result<(), CostLimitExceeded> {
        ...
        if self.budget_exceeded.load(Ordering::Relaxed) {
            ...
            let spent_cents = to_cents(daily.total_usd);
        ...
        // Update daily cost (reset if new day)
        {
            ...
            if let Some(limit_cents) = self.config.max_cost_per_day_cents {
                ...
                if spent_cents >= limit_cents {
                    self.budget_exceeded.store(true, Ordering::Relaxed);
                    warn!(
                        "Daily cost limit reached: ${:.4} of ${:.2}",
                        daily.total_usd,
                        limit_cents as f64 / 100.0
                    );
                } else {
                    let warn_threshold = limit_cents * 80 / 100;
                    if spent_cents >= warn_threshold {
                        warn!(
                            "Approaching daily cost limit: ${:.4} of ${:.2} ({}%)",
                            daily.total_usd,
                            limit_cents as f64 / 100.0,
...
mod tests {
    ...
    #[test]
    fn to_cents_conversion() {
        assert_eq!(to_cents(1.50), 150);
        assert_eq!(to_cents(0.01), 1);
        assert_eq!(to_cents(0.0), 0);
    }

    #[test]
    fn cost_limit_display() {
